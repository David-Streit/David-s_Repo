...
Let's be honest, our application doesn't really solve a real-world problem, but pays homage to the old-school handheld videogame and provides a fun twist on it with various minigames. Our application simulates a digital pet with simple values (hunger, cleanliness, energy, entertainment) that decrease while the player is away. This lets the user see the consequences of their actions (or inaction) and practice taking care of a pet in a playful and time-based way.
...
1. As a player, I want to create a new pet with a name and password so that my progress is saved between sessions.
2. As a player, I want to log in to an existing pet so that I can continue caring for it later.
3. As a player, I want to see my pet's current stats (hunger, cleanliness, energy, entertainment) so that I know which action is needed.
4. As a player, I want to play interactive mini-games (feed, clean, play, sleep) so that I can improve the pet's stats in a fun way.
5. As a player, I want the game to remember when I last played and reduce stats accordingly so that the pet really feels "alive over time".
6. As a player, I want to rename or delete my pet via settings so that I stay in control of my save files.
...
- Pet save files: For each pet, a separate `<name>.txt` file is used to store:
...
- Saving pet data:
The function `safe` writes the current stats and a fresh timestamp back into the text file:
...
- Environment: any standard Python environment with a terminal
...
2. Open a terminal in the project folder (where `main.py` is located).
...
4. Follow the instructions in the console to create or log into a pet and play the game.
...
- `os`
    - Clears the terminal screen to create a cleaner game experience.
    - Checks for the existence of pet save files and deletes them when requested.

- `time`
    - Adds short delays to make animations (loading bars, mini-game transitions) and messages more readable.

- `datetime`
    - Stores and parses timestamps and calculates how many hours passed between sessions so that stats can decay over time.

- `random`
    - Chooses random numbers and words for the different mini-games and events (apples, hide and seek, swipe directions, nightmares, hangman words).
...
| Dominik Schraner |  |
| Samuel Albani |  |
| David Streit| Initial research of possible projects & ReadMe file |

## Contributing 

- Use this repository as a starting point by importing it into your own GitHub account.
- Work only within your own copy.
- do not push to the original template.
